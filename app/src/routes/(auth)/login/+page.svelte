<script lang="ts">
	...
	function onTotpKeydown(i: number, e: KeyboardEvent) {
		if (e.key === 'Backspace') {
			e.preventDefault();
			if (totpDigits[i]) {
				totpDigits = totpDigits.map((d, idx) => idx === i ? '' : d);
			} else if (i > 0) {
				totpDigits = totpDigits.map((d, idx) => idx === i - 1 ? '' : d);
				totpRefs[i - 1]?.focus();
			}
		} else if (/^\d$/.test(e.key) && totpDigits[i]) {
			// box already filled — replace value and advance (maxlength blocks oninput)
			e.preventDefault();
			totpDigits = totpDigits.map((d, idx) => idx === i ? e.key : d);
			if (i < 5) totpRefs[i + 1]?.focus();
		}
	}
