<script lang="ts">
  ...
  import EpisodeList from "$lib/components/anime/EpisodeList.svelte";
  import CharactersRow from "$lib/components/anime/CharectersRow.svelte";
        ...
        <div class="grid grid-cols-[minmax(170px,240px)_1fr] gap-6 items-end">
          ...
          <div class="grid gap-3 min-w-0">
            ...
            {#if details.title_japanese}
              <p class="m-0 text-[1.2rem] text-white/50">{details.title_japanese}</p>
            {/if}
            ...
            <div class="flex flex-wrap gap-1.5">
              ...
              {#if details.season || details.season_year}<span class="chip">{formatAnimeSeason(details.season, details.season_year)}</span>{/if}
            ...
            <div class="flex flex-wrap gap-1.5">
              {#each details.genres as genre}
                <span class="text-[11px] px-2.5 py-1 rounded-full border border-white/10 bg-white/5">{genre}</span>
              {/each}
            </div>
            ...
            <div class="flex gap-2 items-center flex-wrap mt-1">
              <a href="/watch/{details.id}?ep=1"
                ...
                <PlayIcon size={14} weight="fill" /><span>Play</span>
              </a>
              <button type="button"
                class="inline-flex items-center gap-1.5 rounded-[10px] border border-white/15
