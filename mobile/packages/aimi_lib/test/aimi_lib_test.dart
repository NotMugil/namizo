...
void main() {
  ...
  group('AllAnime (StreamProvider)', () {
    test('AllManga provider alias exposes the correct name', () {
      final provider = AllManga();
      expect(provider.name, equals('AllManga'));
      provider.close();
    });
  ...
  group('Anidap (StreamProvider)', () {
    test('search parses response correctly', () async {
      final mockClient = MockClient((request) async {
        if (request.url.path == '/api/anime/search') {
          return http.Response(
            jsonEncode({
              'success': true,
              'data': {
      ...
      final provider = Anidap(client: mockClient);
      final results = await provider.search('Naruto');

      expect(results.length, equals(1));
      expect(results.first.id, equals('20'));
      expect(results.first.title, equals('NARUTO'));
      expect(results.first.availableEpisodes, equals(220));
    });

    test('search returns empty list for zero results', () async {
      final mockClient = MockClient((request) async {
        return http.Response(
          jsonEncode({
            'success': true,
      ...
      final provider = Anidap(client: mockClient);
      final results = await provider.search('unknown');
      expect(results, isEmpty);
    });

    test('getEpisodes parses response and attaches stream', () async {
      final mockClient = MockClient((request) async {
        if (request.url.path == '/api/anime/20/episodes') {
          return http.Response(
            jsonEncode({
              'success': true,
              'data': [
      ...
      expect(episodes.length, equals(2));
      expect(episodes.first.number, equals('1'));
      expect(episodes.first.stream, same(provider));
    });

    test('getSources returns direct url source', () async {
      final mockClient = MockClient((request) async {
        if (request.url.path == '/api/anime/sources') {
          return http.Response(
            jsonEncode({
              'success': true,
              'data': 'https://cdn.example.com/master.m3u8',
            }),
            200,
          );
        }
      ...
      final provider = Anidap(client: mockClient);
      final episode = Episode(animeId: '20', number: '1', stream: provider);
      final sources = await provider.getSources(episode);

      expect(sources.length, equals(1));
      expect(
        sources.first.url,
        contains('https://cdn.example.com/master.m3u8'),
      );
      expect(sources.first.type, equals('hls'));
    });

    test('getSources decodes token payload using tokenDecoder hook', () async {
      final mockClient = MockClient((request) async {
        if (request.url.path == '/api/anime/sources') {
          return http.Response(
            jsonEncode({'success': true, 'data': 'encoded-token'}),
            200,
          );
        }
        return http.Response('Not Found', 404);
      ...
      final provider = Anidap(
        client: mockClient,
        tokenDecoder: (_) async => jsonEncode({
          'sources': [
            {'url': 'https://video.example/master.m3u8', 'quality': '1080p'},
          ],
        }),
      );
      ...
      expect(sources.length, equals(1));
      expect(sources.first.url, contains('https://video.example/master.m3u8'));
      expect(sources.first.quality, equals('1080p'));
    });

    test('getSources uses mode and host options in request', () async {
      final mockClient = MockClient((request) async {
        if (request.url.path == '/api/anime/sources') {
          expect(request.url.queryParameters['type'], equals('dub'));
          expect(request.url.queryParameters['host'], equals('kami'));
          return http.Response(
            jsonEncode({
              'success': true,
      ...
      final provider = Anidap(client: mockClient);
      final episode = Episode(animeId: '20', number: '3', stream: provider);
      final sources = await provider.getSources(
        episode,
        options: {'mode': 'dub', 'host': 'kami'},
      );

      expect(sources.length, equals(1));
      expect(sources.first.type, equals('mp4'));
    });

    test(
      'getSources falls back to alternate host when primary returns proxy url',
      () async {
        final mockClient = MockClient((request) async {
          if (request.url.path == '/api/anime/sources') {
            ...
            if (host == 'yuki') {
              return http.Response(
                jsonEncode({
                  'success': true,
                  'data':
                      'https://cors.anidap.se/media/proxy-token?origin=https://vidwish.live',
                }),
                200,
              );
            }
            if (host == 'nuri') {
              return http.Response(
                jsonEncode({
                  'success': true,
                  'data': 'https://sv5.otakuu.se/storage/test/master.m3u8',
                }),
                200,
              );
            }
        ...
        final sources = await provider.getSources(
          episode,
          options: {'mode': 'sub', 'host': 'yuki'},
        );

        expect(sources.length, equals(1));
        expect(sources.first.url, contains('master.m3u8'));
        expect(sources.first.url, contains('sv5.otakuu.se/storage/'));
      },
    );

    test(
      'getSources falls back to watch page scrape when decode fails',
      () async {
        final mockClient = MockClient((request) async {
          if (request.url.path == '/api/anime/sources') {
            return http.Response(
          ...
          if (request.url.path == '/watch') {
            return http.Response(
              '<media-player src="https://sv5.otakuu.se/storage/test/master.m3u8"></media-player>',
              200,
            );
          }
          return http.Response('Not Found', 404);
        ...
        final provider = Anidap(
          client: mockClient,
          tokenDecoder: (_) async => throw Exception('decode failure'),
        );
        final episode = Episode(animeId: '20', number: '1', stream: provider);
        final sources = await provider.getSources(episode);
        ...
        expect(
          sources.first.url,
          contains('https://sv5.otakuu.se/storage/test/master.m3u8'),
        );
      },
    );

    test(
      'getSources returns empty list on source + fallback failure',
      () async {
        final mockClient = MockClient((request) async {
          if (request.url.path == '/api/anime/sources') {
            return http.Response('Error', 500);
