...
>
	...
	<div class="flex items-center gap-3 min-w-0">
		...
		{#if breadcrumb.items.length > 0}
			<nav class="hidden sm:flex items-center gap-1 min-w-0" aria-label="Breadcrumb">
				{#each breadcrumb.items as item, i}
					...
					{#if item.href && i < breadcrumb.items.length - 1}
						<a href={item.href}
							class="text-[13px] text-white/40 hover:text-white/70 transition-colors truncate max-w-[18ch]"
						>{item.label}</a>
					{:else}
						<span class="text-[13px] text-white/85 font-medium truncate max-w-[22ch]">{item.label}</span>
					{/if}
				{/each}
			</nav>
		{:else}
