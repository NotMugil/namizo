<script lang="ts">
    ...
    $: sorted = order === 'asc' ? normalized : [...normalized].reverse()
    ...
    $: inRange = sorted.filter(number => number > bounds.start && number <= bounds.end)
...
<div class="flex h-full min-h-0 flex-col gap-2">
    <div class="flex flex-wrap items-center gap-1.5">
        <div class="relative min-w-[170px] flex-1">
            ...
            />
            <input
                class="h-8 w-full rounded-md border border-white/8 bg-white/5 pl-7 pr-2
                       text-[0.78rem] text-white outline-none placeholder:text-white/30
        ...
        >
            <Select.Trigger
                class="h-8 min-w-[96px] rounded-md border border-white/8 bg-white/5 px-2
                       text-[0.74rem] text-white/80"
            >
        ...
        <button
            class="h-8 rounded-md border border-white/8 bg-white/5 px-2.5 text-[0.72rem]
                   text-white/60 transition-colors hover:bg-white/8"
    ...
    <div class="flex-1 min-h-0">
        {#if loading}
            <div class="grid h-full min-h-0 content-start gap-1.5 overflow-y-auto pr-0.5
                        grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7">
                {#each Array.from({ length: skeletonCount }) as _, index}
                    <div class="h-10 animate-pulse rounded-md bg-white/7" data-skeleton={index}></div>
                {/each}
            </div>
        {:else if filtered.length === 0}
            <div class="flex h-full items-start">
                <p class="rounded-md bg-white/6 px-3 py-2 text-[0.78rem] text-white/55">
        ...
        {:else}
            <div class="grid h-full min-h-0 content-start gap-1.5 overflow-y-auto pr-0.5
                ...
                {#each filtered as number}
                    ...
                    <button
                        class="h-10 w-full rounded-md text-[0.8rem] font-medium transition-colors
                        {isActive
    ...
    <div class="mt-auto flex items-center justify-end gap-2 text-[0.65rem] text-white/35">
