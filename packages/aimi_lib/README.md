...
## Features
 ...
 - **Stream Provider**: Search and fetch episodes from AllAnime, AnimePahe, Anizone, and Anidap.
...
To test Anidap, run the same CLI and choose `Anidap` when prompted for stream provider.
...
void main() async {
  ...
  final stream = AllManga(); // or AllAnime(), AnimePahe(), Anizone(), Anidap()
...
### Anidap With Host/Mode Options

Use `mode` (`sub`/`dub`) and `host` (provider server) in `getSources`.
...
void main() async {
  final stream = Anidap();
  final results = await stream.search('Naruto');
  final anime = results.first;
  final episodes = await anime.getEpisodes();
  final sources = await episodes.first.getSources(
